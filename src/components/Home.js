...
const stepsData = [
  {
    ...
    description:
      "Grounded in local culture and realities to ensure relevant, accurate, and respectful outcomes.",
  },

  {
    ...
    description:
      "We work hand-in-hand with local organizations, building trust and ensuring our work has a lasting impact.",
  },

  {
    ...
    description:
      "Leveraging rigorous data analysis to provide clear, actionable insights that drive effective decision-making.",
  },
];
...
const AnimatedApproach = () => {
  ...
  const stepPositions = useMemo(() => {
    ...
    const angleStep = 360 / config.numSteps;

    const dotPlacementRadius = config.arcRadius + config.dotOffset;
    ...
    for (let i = 0; i < config.numSteps; i++) {
      ...
      const x = centerX + dotPlacementRadius * Math.cos(angleRad);

      const y = centerY + dotPlacementRadius * Math.sin(angleRad);

      positions.push({ x, y });
    }

    return positions;
  }, [config.numSteps, centerX, centerY, config.arcRadius, config.dotOffset]);

  useEffect(() => {
    const angleStep = 360 / config.numSteps;

    const targetRotation = -(activeStep * angleStep);

    let frameId; // Use a local variable for the animation frame
    ...
    const animate = () => {
      ...
      state.currentRotation +=
        (targetRotation - state.currentRotation) * easingFactor;

      if (arcGroupRef.current) {
        arcGroupRef.current.setAttribute(
          "transform",
          `rotate(${state.currentRotation}, ${centerX}, ${centerY})`
        );
      }
  ...
  }, [activeStep, centerX, centerY, config.numSteps]);

  const activeContent = stepsData[activeStep];

  const foreignObjectSize = config.arcRadius * 1.6;

  const foreignObjectPos = centerX - foreignObjectSize / 2;

  const lineStartY = centerY - foreignObjectSize / 5;

  const lineEndY = centerY - config.arcRadius;

  // Note: Tailwind classes are used here. Make sure Tailwind is set up in your project.

  return (
    <div className="w-full max-w-4xl mx-auto flex flex-col items-center text-white p-6 rounded-lg bg-[#2c4e8a]">
      <h2 className="text-4xl md:text-5xl font-bold mb-8 self-start text-left our-approach">
        Our Approach
      </h2>
            ...
            {stepPositions.map((pos, index) => (
              ...
              >
                <circle
                  ...
                  fill="none"
                  stroke="white"
                  strokeWidth={activeStep === index ? 4 : 2}
